...
from Baselines.BERTMLP import BERTMLPModel, BERTDataset
...
def train_epoch(args: argparse.Namespace, model, dataloader, criterion, optimizer, device):
    ...
    for post_ids, post_mask, labels in tqdm(dataloader, desc="Training"):
        ...
        optimizer.zero_grad()
        logits = model(post_ids, post_mask)
...
def parse_options(name: str = "BERT"):
    ...
    parser.add_argument("--logger_path", type=str, default="./Results/BERT/bert_Essays_MSE", help="The logging path")
...
def train_model(args: argparse.Namespace):
    ...
    dataset = BERTDataset(args, text_list, labels)

    # 初始化模型
    # model = BERT_Model(args).to(device)
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
        ...
        # 初始化模型
        model = BERTMLPModel(args).to(device)
        # 创建数据子集和数据加载器
